...
function CartPage() {
  ...
  const handleRemoveFromCart = async (productId) => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/e-commerce/cart/${productId}`,
        {
          method: "DELETE",
          headers: {
  ...
  const fetchCartData = () => {
    fetch(`${process.env.REACT_APP_API_URL}/e-commerce/cart/view`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  const handlePlaceOrder = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/e-commerce/place-order`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (response.ok) {
        const userResponse = await fetch(
          `${process.env.REACT_APP_API_URL}/e-commerce/details`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
